...
abstract class CityRemoteDataSource {
  Future<ApiResponseModel<List<City>>> get();
}
...
class CityRemoteDataSourceImpl implements CityRemoteDataSource {
  ...
  @override
  Future<ApiResponseModel<List<City>>> get() async {
    final response = await _httpModule.get(ApiEndpoint.city());

    final result = ApiResponseModel<List<City>>.fromJson(
      response,
      (json) {
        if (json is! List) {
          return [];
        ...
        return json.map((e) => City.fromJson(e as JSON)).toList();
      },
    );
